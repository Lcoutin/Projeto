...
// const stepsOn = document.querySelectorAll('.step.on');
let totalStepsOn = document.querySelectorAll('.step.on').length;
...
function updateTotalStepsOn() {
  totalStepsOn = document.querySelectorAll('.step.on').length;
}
...
function nextStep() {
  ...
  function next(width) {
    ...
    nextIndexStep = nextIndexStep + 1;
  }

  if (nextIndexStep < totalStepsOn) {
    let width = (nextIndexStep / totalStepsOn) * 100;
    next(width);
  } else if (nextIndexStep >= totalStepsOn) {
    form.submit();
  }

  console.log(`Index Atual : ${indexStep} e Próximo Index : ${nextIndexStep}`);
  console.log(`Total On : ${totalStepsOn}`);
...
function updateColorCheckedOption() {
...
musicOption.addEventListener("click", updateColorCheckedOption);
contractorOption.addEventListener("click", updateColorCheckedOption);

function validateMusicOption() {
  const stepsHidden = document.querySelectorAll('.step.hidden-step');
  if (musicOption.checked) {
    stepsHidden.forEach(stepHidden => {
      if (stepHidden.classList.contains("hidden-step")) {
        stepHidden.classList.remove("hidden-step");
        stepHidden.classList.add("on");
      }
    });
    return true;
  } else if (!musicOption.checked && contractorOption.checked) {
    steps.forEach(steps => {
      if (!steps.classList.contains("default")) {
        steps.classList.add("hidden-step");
